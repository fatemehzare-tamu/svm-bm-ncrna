...
fprintf('k= %d\n',k);
N=100;
[trainedClassifier1, ~] = treebag1(Dtrain(1:N,:));
% [trainedClassifier2, ~] = treeadaboost(Dtrain(1:N,:));
% [trainedClassifier3, ~] = treeRUS(Dtrain(1:N,:));
[trainedClassifier4, ~] = treegentle(Dtrain(1:N,:));
yfit = trainedClassifier1.predictFcn(Dtest(:,2:9));
[~,~,~,~,MR1(k)] = misclassification_rates(yfit,Dtest(:,1));

% yfit = trainedClassifier2.predictFcn(Dtest(:,2:9));
% [~,~,~,~,MR2(k)] = misclassification_rates(yfit,Dtest(:,1));

% yfit = trainedClassifier3.predictFcn(Dtest(:,2:9));
% [~,~,~,~,MR3(k)] = misclassification_rates(yfit,Dtest(:,1));

yfit = trainedClassifier4.predictFcn(Dtest(:,2:9));
[~,~,~,~,MR4(k)] = misclassification_rates(yfit,Dtest(:,1));
...
% plot(1:30,MR3,'r--o')
plot(1:30,MR4,'m--o')
legend('Bag Method','GentleBoost Method' )
